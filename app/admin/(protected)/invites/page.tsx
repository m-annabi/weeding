...
const STATUS_BADGE = {
  ...
  declined: { label: "❌ Absent", cls: "bg-majorelle/10 text-charcoal border-majorelle/40" },
  pending: { label: "⏳ En attente", cls: "bg-sand text-charcoal/60 border-linen" },
};
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="font-serif text-3xl text-charcoal">Invités & groupes</h1>
      ...
      <section className="rounded-2xl border border-linen bg-white/80 p-6">
        ...
        >
          <div className="flex flex-col gap-1">
            <label className="text-xs text-charcoal/60">Prénom *</label>
            <input name="firstName" required className={inputCls} />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs text-charcoal/60">Nom *</label>
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs text-charcoal/60">Téléphone</label>
            <input name="phone" className={inputCls} />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs text-charcoal/60">Nb pers. max</label>
            <input
              name="maxGuests"
              type="number"
              min={1}
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs text-charcoal/60">Groupe</label>
            <div className="flex gap-2">
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm">
        <span className="text-charcoal/50">Filtrer :</span>
        <Link
          href={filterUrl({ status: undefined })}
          className={`rounded-full border px-3 py-1 ${!statusFilter ? "border-charcoal bg-charcoal text-ivory" : "border-charcoal/20 bg-white/70 hover:border-charcoal"}`}
        >
          Tous
        </Link>
        {(
          ...
          <Link
            ...
            className={`rounded-full border px-3 py-1 ${statusFilter === value ? "border-charcoal bg-charcoal text-ivory" : "border-charcoal/20 bg-white/70 hover:border-charcoal"}`}
          >
            {label}
          </Link>
        ))}
        ...
        <Link
          ...
          className={`rounded-full border px-3 py-1 ${!groupFilter ? "border-majorelle bg-majorelle text-ivory" : "border-charcoal/20 bg-white/70 hover:border-majorelle"}`}
        >
          Tous les groupes
        </Link>
        {groups.map((g) => (
          <Link
            ...
            className={`rounded-full border px-3 py-1 ${groupFilter === g.name ? "border-majorelle bg-majorelle text-ivory" : "border-charcoal/20 bg-white/70 hover:border-majorelle"}`}
          >
      ...
      <div className="overflow-x-auto rounded-2xl border border-linen bg-white/80">
        <table className="w-full text-sm">
          ...
          <tbody>
            {guests.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-charcoal/40 font-light">
              ...
              return (
                <tr key={g.id} className="border-b border-linen/60 last:border-0">
                  ...
                  <td className="px-4 py-3 font-light">
                    {g.group?.name ?? <span className="text-charcoal/30">—</span>}
                  </td>
                  <td className="px-4 py-3 font-light text-charcoal/70">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1.5">
                      ...
                      <a
                        ...
                        className="rounded-full border border-charcoal/20 px-3 py-1 text-xs hover:border-majorelle hover:text-majorelle transition"
      ...
      <section className="rounded-2xl border border-linen bg-white/80 p-6">
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          {groups.map((g) => (
            ...
            >
              {g.name}
              <span className="text-charcoal/50">({g._count.guests})</span>
              <form action={deleteGroup} className="inline">
                <input type="hidden" name="id" value={g.id} />
                <button
                  className="text-charcoal/40 hover:text-majorelle transition"
