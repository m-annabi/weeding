...
French-language wedding website ("weeding" is a typo kept as the repo name): public site (full-screen hero-video homepage + `/infos`, `/programme`, `/contacts` tabs), per-guest RSVP forms accessed via unique token links/QR codes, and a password-protected couple's dashboard. UI copy is entirely in French.
...
- **`content/wedding.ts`** — single source of truth for all wedding content (names, date, venue, schedule, contacts, airports/travel tips, homepage story blocks, and paths to media in `public/images` / `public/videos`). Edit here, never hardcode wedding facts in components.
- **Shared UI** (`components/`): `site-nav.tsx` (fixed nav, transparent over the homepage hero then frosted on scroll — pass `overlay` on pages with a full-bleed hero), `site.tsx` (Card, Divider, PageHeader, SiteFooter, schedule icon mapping), `ornaments.tsx` (inline SVG decorations/icons). Public pages compose these; reuse them instead of restyling per page.
- **Data model** (`prisma/schema.prisma`): `Group` → `Guest` (has unique `token` used as the personal RSVP URL, and `maxGuests` = seats covered by the invitation) → `Rsvp` (1:1 with Guest) → `Participant` (one row per attendee; `diet` is a string enum NONE/VEGETARIAN/VEGAN/HALAL/GLUTEN_FREE/OTHER — a holdover from the original SQLite setup, kept as a string on Postgres). `Rsvp` also carries travel data (arrivalMode PLANE/CAR/ON_SITE, airport, flight dates/times, `accommodation` KASBAH/OTHER = wants couple-provided lodging or self-organized). `Lodging` (name + capacity in persons) is assigned per invitation via `Guest.lodgingId` in `/admin/hebergements`.
...
- **Theme**: Tailwind v4 tokens in `app/globals.css` (`@theme` for the « riad contemporain / quiet luxury » palette: ivory, sand, linen, champagne, brass, majorelle, majorelle-deep, charcoal, olive, sage — Majorelle blue is the accent, browns are banned). Fonts are Fraunces (serif) and Jost (sans), loaded via `next/font` variables on `<body>`; font tokens referencing them **must stay in `@theme inline`** (plain `@theme` silently drops `var()` references). `.smallcaps` is the shared label style; `.script` is italic serif (no handwriting font).
