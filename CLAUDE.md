...
- **Theme**: Tailwind v4 tokens in `app/globals.css` (`@theme` for the terracotta/Marrakech palette: cream, sand, linen, nude, camel, toffee, terracotta, sienna, cocoa, olive, sage). Fonts are Fraunces (serif), Jost (sans) and Caveat (`.hand`), loaded via `next/font` variables on `<body>`; font tokens referencing them **must stay in `@theme inline`** (plain `@theme` silently drops `var()` references). `.smallcaps` is the shared label style.
